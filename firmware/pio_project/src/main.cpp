...
void setup() {
  ...
  keypad.setDebounceTime(100);   // 100ms debounce (default is 10ms — too fast for noisy lines)
  keypad.setHoldTime(800);       // 800ms hold threshold
  
  // Enable internal pull-ups on column pins to prevent floating/ghost presses
  for (int i = 0; i < COLS; i++) {
    pinMode(colPins[i], INPUT_PULLUP);
  }
  #else
  Serial.println("[Setup] HARDWARE_ATTACHED=false — skipping peripheral init");
  #endif

  // ── Initialize hardware watchdog timer ─────────────────────────
  // Resets ESP32 if loop() hangs for WDT_TIMEOUT_SECONDS.
  // Use version-appropriate API for ESP-IDF 4.4 (Core 2.x) vs 5.x (Core 3.x)
...
// ═══════════════════════════════════════════════════════════════════════
//  FORWARD DECLARATIONS (required for PlatformIO .cpp compilation)
// ═══════════════════════════════════════════════════════════════════════
#if HARDWARE_ATTACHED
...
#endif
void uploadNvsSales();
void saveToNvs(float litres, float amount, const char* paymentType, const char* voucherCode, const char* phone);
...
void loop() {
  ...
  if (key) {
    Serial.printf("[Keypad] Key pressed: '%c' (0x%02X) in state %d\n", key, key, currentState);
  }
...
// ═══════════════════════════════════════════════════════════════════════
//  STATE HANDLERS (only compiled when hardware is attached)
// ═══════════════════════════════════════════════════════════════════════
#if HARDWARE_ATTACHED
